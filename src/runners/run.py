...
from src.models.elph import ELPH, HeFormer
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    tmp_train_res, tmp_val_res, tmp_test_res = result
                    if tmp_test_res > test_res:
                        train_res = tmp_train_res
                        val_res = tmp_val_res
...
def select_model(args, dataset, emb, device):
    if args.model == 'SEALDGCNN':
        ...
        model = SEALGIN(args.hidden_channels, args.num_seal_layers, args.max_z, dataset.num_features,
                        args.use_feature, node_embedding=emb, dropout=args.dropout).to(device)
    elif args.model == 'HeFormer':
        model = HeFormer(args, dataset.num_features, node_embedding=emb).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--former_hidden', type=int, default=256)
    parser.add_argument('--former_dropout', type=float, default=0.5)
    parser.add_argument('--former_num_layer', type=int, default=1)
    parser.add_argument('--former_num_heads', type=int, default=1)
    ...
    # GNN settings
    parser.add_argument('--model', type=str, default='HeFormer')
