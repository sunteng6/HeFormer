...
def get_train_func(args):
    if args.model == 'ELPH':
        ...
        train_func = train_HeFormer
    else:
        train_func = train
    return train_func


def train_HeFormer(model, optimizer, train_loader, args, device, emb=None):
...
def train(model, optimizer, train_loader, args, device, emb=None):
    ...
    for batch_count, data in enumerate(pbar):
        start_time = time.time()
        optimizer.zero_grad()
        # todo this loop should no longer be hit as this function isn't called for HeFormer
        if args.model == 'HeFormer':
