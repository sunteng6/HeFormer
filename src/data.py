...
def get_loaders(args, dataset, splits, directed):
    ...
    if args.model in {'ELPH', 'HeFormer'}:
    ...
    dl = DataLoader if args.model in {'ELPH', 'HeFormer'} else pygDataLoader
    ...
    test_loader = dl(test_dataset, batch_size=args.batch_size, shuffle=shuffle_test,
                     num_workers=args.num_workers)
    if (args.dataset_name == 'ogbl-citation2') and (args.model in {'ELPH', 'HeFormer'}):
